...
class SubscriptionController extends Controller
{
    ...
    {
        // Include disabled markets so existing subscriptions targeting
        // disabled-but-still-defined markets render their market column
        // with the proper name instead of falling back to the raw key.
    ...
    public function unsubscribe($id)
    {
        ...
        if ($subscription->isPluginManaged()) {
            $info = $subscription->getPluginInfo();
            return back()->with('error', "Cannot delete — this subscription is managed by {$info['label']}. Unsubscribe from that plugin's settings instead.");
        }
    ...
    public function clearPlugin(Request $request)
    {
        ...
        // Block clearing plugin-managed subscriptions
        if (array_key_exists($pluginName, TypeSubscription::PLUGIN_MANAGED)) {
            $info = TypeSubscription::PLUGIN_MANAGED[$pluginName];
            return back()->with('error', "Cannot clear — these subscriptions are managed by {$info['label']}. Change the price provider in that plugin's settings to unsubscribe.");
        }

        $count = TypeSubscription::where('plugin_name', $pluginName)->delete();

        return back()->with('success', "Removed {$count} subscription(s) for plugin: " . $pluginName);
    }
